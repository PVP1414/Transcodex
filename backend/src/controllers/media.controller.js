    ...
    if (isVideo) {
      ...
      (async () => {
        try {
          ...
          const videoMeta =
            await videoTranscoder.getVideoMetadata(uploadedFilePath);
          const durationSec =
            videoMeta?.duration != null ? Number(videoMeta.duration) : null;
          if (durationSec && Number.isFinite(durationSec)) {
            media.duration = durationSec;
          }

          const thumbnailPath = await videoTranscoder.generateThumbnail(
            uploadedFilePath,
            media._id.toString(),
          );
...
async function createThumbnail(filePath, originalName, mediaId) {
  ...
  const thumbPath = path.join(
    __dirname,
    `../../uploads/images/${mediaId}/thumbnails/${thumbName}`,
  );
  ...
  for (const variant of sizes) {
    if (originalMeta.width > variant.width) {
      ...
      const variantPath = path.join(
        __dirname,
        `../../uploads/images/${mediaId}/variants/${variantName}`,
      );
